...
public class RegisterPageController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		cancelButton.setOnMouseClicked(e -> {
			try {
				SceneChangingUtils.changeScene(e, "PassLock", "view/HomePage.fxml");
	...
	protected void registerAccount(Event e) {
		...
		if (hasEmptyTextField()) {
			...
			DBUtils.registerAccount(e, firstname, lastname, username, email, passwordEncrypted, securityQuestion,
					securityAnswer);
		}
	}
	...
	protected boolean hasEmptyTextField() {
		if (!firstNameTextField.getText().trim().isEmpty() && !lastNameTextField.getText().trim().isEmpty()
				...
				&& !passwordTextField.getText().trim().isEmpty() && !rePasswordTextField.getText().trim().isEmpty()
	...
	/**
	 * Calls the static method on AccountRequirements class to check the validation
	 * of email.
	 * 
	...
	/**
	 * When users clicks the text field return to the default border color When
	 * users press enter, start register process
	 */
